(() => {
  ...
  function initialize({stage, sourceSvg, oldCanvas, oldControls}) {
    ...
    const state = {
      ...
      selectedId:'star', system:null, hitTargets:[], canvas:null, context:null, flatOverlay:null,
      width:1, height:1, lastFrame:0, previousTimestamp:performance.now(), screenPositions:new Map()
    };
    ...
    function createControls() {
      ...
      panel.innerHTML = `
        ...
        <div id="exo-camera-controls" class="exo-camera-controls" hidden>
          ...
          <label><span>Spatial zoom</span><input id="exo-exclusive-zoom" type="range" min="${MIN_ZOOM_PERCENT}" max="${MAX_ZOOM_PERCENT}" step="1" value="100"></label>
        </div>
        <p class="exo-orbit-architecture-note">Published orbital elements are used whenever present. Camera, body, and epoch transitions are tweened rather than snapped.</p>`;
    ...
    function bind() {
      ...
      $('exo-exclusive-yaw')?.addEventListener('input', event => { state.targetYaw = Number(event.target.value); });
      $('exo-exclusive-pitch')?.addEventListener('input', event => { state.targetPitch = Number(event.target.value); });
      $('exo-exclusive-zoom')?.addEventListener('input', event => { state.targetZoom = clamp(Number(event.target.value) / 100, MIN_ZOOM_PERCENT / 100, MAX_ZOOM_PERCENT / 100); });
    ...
    function render(timestamp) {
      ...
      const cameraAlpha = 1 - Math.exp(-elapsed * 10);
      state.displayYaw = tweenAngleDegrees(state.displayYaw, state.targetYaw, cameraAlpha);
      state.displayPitch += (state.targetPitch - state.displayPitch) * cameraAlpha;
      state.displayZoom += (state.targetZoom - state.displayZoom) * cameraAlpha;
      globalThis.BlacklightExoCameraState = {
        yaw:state.displayYaw * Math.PI / 180,
        pitch:state.displayPitch * Math.PI / 180,
        zoom:state.displayZoom
      };
      if (timestamp - state.lastFrame >= 33) {
        state.lastFrame = timestamp;
        if (state.view === '3d') draw(elapsed);
      }
      requestAnimationFrame(render);
    }
    ...
    function draw(elapsed) {
      ...
      const scale = Math.min(width, height) * .42 / outer * state.displayZoom;
      const epoch = Number(globalThis.BlacklightExoGetProjectionEpochDays?.() ?? globalThis.BlacklightExoProjectionEpochDays ?? 0);
      ...
      for (const body of system.planets) {
        ...
        const anomaly = elements.phase + epoch / Math.max(.01, Number(body.periodDays) || 1) * TAU;
        const model = orbitPoint(Number(body.distance) || 0, elements, anomaly);
        const targetProjected = project(model, width, height, scale);
        const projected = tweenScreen(body.id, targetProjected, elapsed);
        ...
        for (const moon of body.moons) {
          ...
          const moonAnomaly = moonElements.phase + epoch / Math.max(.01, Number(moon.periodDays) || 1) * TAU;
      ...
      for (const body of bodies) {
        context.beginPath(); context.arc(body.x, body.y, body.size, 0, TAU);
        context.fillStyle = body.color; context.shadowColor = body.color; context.shadowBlur = body.kind === 'moon' ? 1 : 4; context.fill(); context.shadowBlur = 0;
    ...
    function tweenScreen(id, target, elapsed) {
      ...
      const alpha = 1 - Math.exp(-elapsed * 13);
      const current = {
        x:previous.x + (target.x - previous.x) * alpha,
        y:previous.y + (target.y - previous.y) * alpha,
        z:previous.z + (target.z - previous.z) * alpha,
        perspective:previous.perspective + (target.perspective - previous.perspective) * alpha
      };
      state.screenPositions.set(id, current);
      return current;
    }
    ...
    function elementsFor(object) {
      ...
      return {eccentricity:rng() * .2, inclination:rng() * .2, ascendingNode:rng() * TAU, periapsis:rng() * TAU, phase:number(object.phase, rng() * TAU), verticalWarp:(rng() - .5) * .02};
    }
    ...
    function drawMoonOrbit(context, parent, radius, elements, width, height, scale) {
      const points = [];
      for (let step = 0; step < 48; step += 1) { const rel = orbitPoint(radius, elements, step / 48 * TAU); points.push(project({x:parent.model.x + rel.x, y:parent.model.y + rel.y, z:parent.model.z + rel.z}, width, height, scale)); }
      path(context, points, 'rgba(199,210,220,.42)', [3,4]);
    }
    function path(context, points, stroke, dash = []) { if (!points.length) return; context.beginPath(); context.moveTo(points[0].x, points[0].y); for (let i = 1; i < points.length; i += 1) context.lineTo(points[i].x, points[i].y); context.closePath(); context.strokeStyle = stroke; context.lineWidth = 1; context.setLineDash(dash); context.stroke(); context.setLineDash([]); }
    function drawHabitable(context, system, width, height, scale) { for (const radius of [system.star.hzInner, system.star.hzOuter]) { const points = [], elements = {eccentricity:0, inclination:0, ascendingNode:0, periapsis:0, verticalWarp:0}; for (let step = 0; step < 80; step += 1) points.push(project(orbitPoint(radius, elements, step / 80 * TAU), width, height, scale)); path(context, points, 'rgba(82,194,123,.38)', [4,5]); } }
    function drawStar(context, width, height, scale) {
      const star = project({x:0, y:0, z:0}, width, height, scale);
      const gradient = context.createRadialGradient(star.x, star.y, 1, star.x, star.y, 15);
      gradient.addColorStop(0, '#fff8d9'); gradient.addColorStop(.32, '#ffd36b'); gradient.addColorStop(1, 'rgba(217,168,79,0)');
      context.fillStyle = gradient; context.beginPath(); context.arc(star.x, star.y, 15, 0, TAU); context.fill();
      context.fillStyle = '#ffd36b'; context.beginPath(); context.arc(star.x, star.y, 4.5, 0, TAU); context.fill();
      state.hitTargets.push({id:'star', x:star.x, y:star.y, radius:9, depth:star.z});
    }
    function drawStarfield(context, width, height, seed) { const rng = randomFor(`${seed}:3d-starfield`); context.fillStyle = 'rgba(220,235,245,.6)'; for (let index = 0; index < 90; index += 1) { const x = rng() * width, y = rng() * height, r = .25 + rng() * 1.1; context.beginPath(); context.arc(x, y, r, 0, TAU); context.fill(); } }
    function bodySize(body, perspective) { if (body.kind === 'dwarf-planet') return Math.max(3.5, 5 * perspective); if (/Gas giant/.test(body.type)) return Math.max(6, 11 * perspective); if (/Ice giant/.test(body.type)) return Math.max(5, 8 * perspective); return Math.max(4, 6 * perspective); }
    function distanceRange(moons) { const values = moons.map(moon => Number(moon.orbitalDistanceKm)).filter(value => value > 0); return {min:Math.min(...values, 1), max:Math.max(...values, 1)}; }
    function moonVisualAu(moon, range, outer) { const distance = Number(moon.orbitalDistanceKm); const t = distance > 0 && range.max > range.min ? (Math.log(distance) - Math.log(range.min)) / (Math.log(range.max) - Math.log(range.min)) : .5; return outer * (.012 + t * .045); }
    ...
    function tweenAngleDegrees(current, target, alpha) { let delta = (target - current) % 360; if (delta > 180) delta -= 360; if (delta < -180) delta += 360; return current + delta * alpha; }
  }
