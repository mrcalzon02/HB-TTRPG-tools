(() => {
  ...
  let clusterReady=false,clusterLoading=null,routeReady=false,imageryReady=false;
  ...
  function load(src){
    ...
    const promise=new Promise((resolve,reject)=>{
      ...
      if(existing?.dataset.blacklightLoaded==='true'){resolve();return;}
      const script=existing||document.createElement('script');
      const complete=()=>{script.dataset.blacklightLoaded='true';resolve();};
  ...
  async function loadCore(){
    status('Loading the lightweight system renderer…','working');setDisabled(buttonIds,true);
  ...
  async function loadEcologyAndImagery(){
    ...
    const button=document.getElementById('exo-load-imagery-ecology');if(button)button.disabled=true;
    ...
    try{
      ...
      imageryReady=true;if(button)button.textContent='Imagery and Ecology Loaded';
    ...
    }catch(error){console.warn('[Blacklight EXO] Optional imagery/ecology layer unavailable:',error);status(`Imagery and ecology failed without stopping system inspection: ${error.message}`,'error');if(button)button.disabled=false;}
  }

  async function loadCluster(){
    if(clusterReady)return;
    if(clusterLoading)return clusterLoading;
    const controls=['exo-generate-cluster','exo-random-cluster','exo-force-populated-hz'];setDisabled(controls,true);
    status('Loading cluster generation on demand…','working');
    globalThis.BlacklightExoDeferClusterAutostart=true;
  ...
  function addActionButton(id,text,handler){
    const actions=document.querySelector('.exo-system-actions');if(!actions||document.getElementById(id))return;
    const button=document.createElement('button');button.id=id;button.type='button';button.className='bli-action';button.textContent=text;button.addEventListener('click',handler);actions.append(button);
  }
  ...
  function installLazyControls(){
    ...
    if(host&&!document.getElementById('exo-load-route-tools')){const button=document.createElement('button');button.id='exo-load-route-tools';button.type='button';button.className='bli-action';button.textContent='Load Route and Jump Tools';button.addEventListener('click',loadRoutes);host.append(button);}
  }
  ...
  async function start(){
    try{
      await loadCore();installLazyControls();monitorCatalogue();idle(optionalHandoffs);
    }catch(error){
