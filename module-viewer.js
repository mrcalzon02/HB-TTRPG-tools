(() => {
  ...
  async function drawMap(root){
    ...
    wrap.innerHTML='';
    if(String(image).toLowerCase().endsWith('.svg')){
      try{
        const svgText=await fetch(image,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(image);return r.text()});
  ...
  function doorGroupById(groupId){const key=groupId.replace('door-group:','');return doorGroups().get(key)||[]}
  ...
  function drawHotspots(root){
    ...
    if(!showDoors)return;
    doorGroups().forEach((group,key)=>{
      const h=group[0];
      const groupId=group.length>1?`door-group:${key}`:h.targetId;
      const label=group.length>1?String(group.length):'';
      renderHotspot(m,h,groupId,label,group.length>1);
    });
  }
  ...
  function renderHotspot(m,h,targetId,label,grouped=false){
    ...
    b.onclick=()=>{selectedId=targetId;detail(document.getElementById('module-viewer-root'));list(document.getElementById('module-viewer-root'));drawHotspots(document.getElementById('module-viewer-root'))};
    m.appendChild(b);
  }
  ...
  function doorLinks(room){const doorIds=room.doorIds||[];const exact=doorIds.map(id=>doorById(id)).filter(Boolean);const doors=exact.length?exact:(moduleData.doors||[]).filter(d=>d.from===room.id||d.to===room.id);return `<h4>Listed doors and entries</h4><div class="module-pill-row">${doors.map(d=>`<button type="button" class="module-pill ${(d.tags||[]).join(' ')}" data-door="${esc(d.id)}">${esc(d.label)}<span class="module-door-summary">${esc(d.kind)}</span></button>`).join('')}</div>`}
