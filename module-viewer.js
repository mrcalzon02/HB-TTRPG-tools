(() => {
  ...
  async function loadModule(path){
    ...
    for(const patchPath of MODULE_PATCHES[data.id] || []){
      try{ applyPatch(data, await getJson(patchPath)); }
      catch(e){ console.warn('Module patch failed', patchPath, e); }
    }
    return data;
  }
  ...
  function loadEditorScript(){
    if(document.querySelector(`script[src="${EDITOR_SCRIPT}"]`)) return;
  ...
  async function drawMap(root){
    const wrap=root.querySelector('#module-map');
    const image=moduleData.map?.image || '';
    wrap.innerHTML='';
    if(image.toLowerCase().endsWith('.svg')){
      try{ wrap.innerHTML=await getText(image); const svg=wrap.querySelector('svg'); if(svg){ svg.removeAttribute('width'); svg.removeAttribute('height'); svg.setAttribute('preserveAspectRatio','xMidYMid meet'); } }
      catch(e){ wrap.innerHTML=`<img src="${esc(image)}" alt="${esc(moduleData.title)} map" />`; }
    } else {
      wrap.innerHTML=`<img src="${esc(image)}" alt="${esc(moduleData.title)} map" />`;
    }
    drawHotspots(root);
  }
  ...
  function drawHotspots(root){
    const m=root.querySelector('#module-map');
    if(!m) return;
    m.querySelectorAll('.module-hotspot').forEach(n=>n.remove());
    (moduleData.hotspots||[]).filter(h=>h.type!=='door').forEach(h=>{ if(h.type==='room'&&!showRooms) return; renderHotspot(m,h,h.targetId,h.type==='room'?h.label:''); });
    if(!showDoors) return;
    doorGroups().forEach((group,key)=>{ const h=group[0]; const groupId=group.length>1?`door-group:${key}`:h.targetId; renderHotspot(m,h,groupId,group.length>1?String(group.length):'',group.length>1); });
  }

  function renderHotspot(m,h,targetId,label,grouped=false){
    const b=document.createElement('button');
    b.type='button';
    b.className=`module-hotspot ${h.type} ${grouped?'grouped':''} ${selectedId===targetId?'active':''}`;
    Object.assign(b.style,{left:h.box.x+'%',top:h.box.y+'%',width:h.box.w+'%',height:h.box.h+'%'});
    b.textContent=label;
    b.title=grouped?`${label} listed entries at this physical doorway`:(h.label||h.targetId);
    b.onclick=()=>{ selectedId=targetId; const root=document.getElementById('module-viewer-root'); detail(root); list(root); drawHotspots(root); };
    m.appendChild(b);
  }

  function list(root){
    const target=root.querySelector('#module-list');
    target.innerHTML='';
    [...(moduleData.rooms||[]).map(r=>({id:r.id,title:r.title,meta:r.summary,kind:'room'})),...(moduleData.doors||[]).map(d=>({id:d.id,title:d.label,meta:d.kind,kind:'door'}))].forEach(x=>{
      const b=document.createElement('button'); b.type='button'; b.className=selectedId===x.id?'active':''; b.innerHTML=`<strong>${esc(x.title)}</strong><small>${esc(x.kind)} · ${esc(x.meta||'')}</small>`; b.onclick=()=>{ selectedId=x.id; detail(root); list(root); drawHotspots(root); }; target.appendChild(b);
    });
  }

  function detail(root){
    const t=root.querySelector('#module-detail');
    if(String(selectedId||'').startsWith('door-group:')){ const group=doorGroupById(selectedId); t.innerHTML=`<p class="eyebrow">Physical doorway</p><h3>${group.length} listed room-side entries share this map position</h3><p>The printed map shows one doorway here, while the room text lists one or more room-side entries for that same physical position.</p><div class="module-pill-row">${group.map(h=>doorById(h.targetId)).filter(Boolean).map(d=>doorButton(d)).join('')}</div>`; return; }
    const room=(moduleData.rooms||[]).find(r=>r.id===selectedId);
    const door=(moduleData.doors||[]).find(d=>d.id===selectedId);
    if(room){ t.innerHTML=`<p class="eyebrow">Room ${esc(room.number)}</p><h3>${esc(room.title)}</h3><p>${esc(room.summary)}</p>${section('Features',room.features)}${section('Traps',room.traps)}${section('Tricks',room.tricks)}${section('Monsters',room.monsters)}${section('Treasure',room.treasure)}${doorLinks(room)}${rawSection('Source room listing',room.sourceText)}`; return; }
    if(door){ t.innerHTML=`<p class="eyebrow">Door / Entry</p><h3>${esc(door.label)}</h3><div class="module-pill-row">${(door.tags||[]).map(x=>`<span class="module-pill ${esc(x)}">${esc(x)}</span>`).join('')}</div><div class="module-stat-grid"><div class="module-stat"><strong>From</strong>${esc(roomName(door.from))}</div><div class="module-stat"><strong>To</strong>${esc(roomName(door.to))}</div><div class="module-stat"><strong>Type</strong>${esc(door.kind)}</div><div class="module-stat"><strong>Notes</strong>${esc(door.notes)}</div></div>${rawSection('Source door listing',door.sourceText)}`; return; }
    t.innerHTML=`<h3>${esc(moduleData.title)}</h3><p>${esc(moduleData.source?.notes||'')}</p>`;
  }
