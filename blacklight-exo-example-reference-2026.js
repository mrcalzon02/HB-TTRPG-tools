(() => {
  ...
  function updatePublicReference() {
    const item = globalThis.BlacklightExoExampleReference?.systems
      ?.find(system => system.name === 'Epsilon Indi');
    if (!item) return;
    item.confirmed = '1 confirmed directly imaged cold super-Jupiter around Epsilon Indi A; two dynamically measured brown-dwarf companions are also present';
    item.knownOrbitingMassEarth = EPSILON_INDI_PLANET_EARTH;
    item.confidence = '2026 JWST/astrometry estimate: 7.6 ± 0.7 Jupiter masses';
    item.material = 'JWST photometry confirms atmospheric ammonia. Thick water-ice clouds are the favored explanation for the suppressed ammonia feature, though low metallicity or nitrogen depletion remain alternatives.';
    item.note = 'Cluster gravity includes Epsilon Indi A, the measured Ba/Bb brown-dwarf masses, and the updated planet mass.';
  }
  ...
  function apply() {
    ...
    const card = [...document.querySelectorAll('#exo-cluster-grid .exo-cluster-card')]
      .find(item => (item.dataset.catalogName || item.querySelector('h3')?.textContent.trim()) === 'Epsilon Indi');
    if (!card) return;
    ...
    const planetaryMassSolar = EPSILON_INDI_PLANET_EARTH / EARTHS_PER_SOLAR_MASS;
    const existing = masses.get(seed) || {};
    const record = {
      ...existing,
      seed,
      stellarMassSolar: EPSILON_INDI_SUBSTELLAR_SOLAR,
      planetMassEarth: EPSILON_INDI_PLANET_EARTH,
      moonMassEarth: 0,
      beltMassLunar: 0,
      planetaryMassSolar,
      beltMassSolar: 0,
      totalSolarMass: EPSILON_INDI_SUBSTELLAR_SOLAR + planetaryMassSolar,
      provenance: '2026-published-reference',
      referenceUpdated: '2026-07-13'
    };
    masses.set(seed, record);
    card.dataset.stellarMass = String(EPSILON_INDI_SUBSTELLAR_SOLAR);
    ...
    const values = new Map([
      ...
      ['Total gravity-source mass', `${record.totalSolarMass.toFixed(7)} M☉`],
