(() => {
  ...
  function generateProceduralSystem(seed) {
    ...
    const star = {
      id:'star',kind:'star',name,class:starClass.class,label:starClass.label,
      mass:randomNumber(rng,...starClass.mass,3),luminosity:randomNumber(rng,...starClass.luminosity,4),
      temperature:integer(rng,...starClass.temperature),color:starClass.color,age:randomNumber(rng,.1,11.8,2),
      resources:['continuous fusion output','stellar-wind collection'],hazards:['stellar radiation'],provenance:'rng'
    };
    ...
    return {version:5,seed,sourceMode:'procedural',provenance:'rng',name,star,planets,belts,majorPlanetCount:planets.length,dwarfPlanetCount:0,moonCount:moons.length,resourceTotals:{metals:planets.length+belts.length,volatiles:planets.filter(body=>/giant|frozen/i.test(body.type)).length,fuel:planets.filter(body=>/giant/i.test(body.type)).length,biospheres:planets.filter(body=>!/No confirmed/.test(body.biosphere)).length,habitable:planets.filter(body=>body.habitability>=65).length,industrial:planets.length+belts.length},features:[`${moons.length} individually charted moons.`,`${belts.length} selectable debris fields charted.`]};
  }
  ...
  function renderAll(){
    ...
    setText($('exo-orbit-svg-desc'),`Corridor-separated orbital projection with ${system.planets.length} heliocentric bodies and ${system.moonCount||0} individually selectable moons.`);
  ...
  function drawOrbits(){
    ...
    for(const body of system.planets){
      ...
      const target=svg('g',{class:`exo-planet-target ${body.kind==='dwarf-planet'?'exo-dwarf-target':''}`,tabindex:'0',role:'button','aria-label':`Select ${body.name}`});
      ...
      for(const moon of body.moons||[]){
        ...
        const title=svg('title');title.textContent=`${moon.name} · moon of ${body.name}`;moonGroup.append(title);bindSelection(moonGroup,moon.id);carrier.append(moonGroup);
    ...
    const star=svg('g',{class:'exo-star-target',tabindex:'0',role:'button','aria-label':`Select ${system.star.name}`});
  ...
  function applyLayout(force=false){
    ...
    for(const body of system.planets){
      ...
      for(const moon of body.moons||[]){
        const moonVisual=visuals.get(moon.id),moonLayout=layout.moonLayouts.get(moon.id);if(!moonVisual||!moonLayout)continue;
        moonVisual.targetRadius=moonLayout.displayRadius;if(force)moonVisual.displayRadius=moonLayout.displayRadius;
        moonVisual.orbit.classList.toggle('is-orbit-visible',moonLayout.orbitVisible);
        moonVisual.label.classList.toggle('is-label-visible',moonLayout.labelVisible);
        moonVisual.group.classList.toggle('is-major-moon',moonLayout.major);
        moonVisual.group.style.opacity=String(moonLayout.pointOpacity);
      }
    }
  ...
  function appendRow(object,cells){const row=document.createElement('tr');row.dataset.objectId=object.id;row.dataset.bodyKind=object.kind;row.dataset.parentId=object.parentId||'';row.dataset.provenance=object.provenance||system.provenance||'unknown';for(const[key,value]of Object.entries({distanceAu:object.distance,orbitalDistanceKm:object.orbitalDistanceKm,periodDays:object.periodDays,eccentricity:object.eccentricity,inclination:object.inclination,phase:object.phase,ascendingNode:object.ascendingNode,argumentOfPeriapsis:object.argumentOfPeriapsis,radiusEarth:object.radius})){if(Number.isFinite(Number(value)))row.dataset[key]=String(value);}row.dataset.bodyColor=object.color||'';row.setAttribute('aria-selected','false');cells.forEach((value,index)=>{const cell=document.createElement('td');if(index===1){const button=document.createElement('button');button.type='button';button.textContent=value;button.addEventListener('click',event=>{event.stopPropagation();select(object.id);});cell.append(button);}else cell.textContent=String(value);row.append(cell);});row.addEventListener('click',event=>{if(!event.target.closest('button'))select(object.id);});ui.table.append(row);}
  ...
  function select(id,inspectRecord=true){const object=objectById(id);if(!object)return;selectedId=id;for(const[key,visual]of visuals)visual.group?.classList.toggle('exo-selected',key===id);for(const row of ui.table.querySelectorAll('tr'))row.setAttribute('aria-selected',String(row.dataset.objectId===id));applyLayout();if(inspectRecord)inspect(object);else inspect(object);document.dispatchEvent(new CustomEvent('blacklight:object-selected',{detail:{id,kind:object.kind,parentId:object.parentId||null}}));}
  ...
  function formatPeriod(value){const parsed=finite(value,NaN);if(!Number.isFinite(parsed))return'Unknown';if(parsed<2)return`${parsed.toFixed(3)} days`;if(parsed<730)return`${parsed.toFixed(2)} days`;return`${(parsed/365.25).toFixed(2)} years`;}
