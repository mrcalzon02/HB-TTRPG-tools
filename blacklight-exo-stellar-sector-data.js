(() => {
  ...
  const gloamCluster=clusters.find(cluster=>cluster.clusterId==='cluster-33');
  if(gloamCluster)gloamCluster.campaignSites=[{
    siteId:'gloam-grave-orbit',
    name:'Gloam Derelict Custody Anchorage',
    aliases:['Gloam Yard','Grave Orbit'],
    systemName:'Gloam Concord System A',
    anchorBody:'Cairn Minor',
    siteClass:'mostly abandoned legacy derelict concentration',
    operationalState:'decommissioned and largely unattended',
    administration:'No permanent active authority. Legacy registries, intermittent autonomous beacons, and distant legal claims survive without routine local enforcement.',
    currentPopulation:'No official permanent population; transient scavengers, squatters, researchers, claimants, and recovery crews normally number from a few dozen to a few hundred across the entire field.',
    majorHullCount:463,
    hullComposition:{commercialFreightAndBulkCargo:176,industrialMiningAndConstruction:84,fleetAuxiliaryTugTenderAndRepair:59,passengerMigrantAndTransport:43,navalCombatant:21,explorationScienceAndSurvey:20,colonyHabitatAndArk:17,customsSecurityPatrolAndCourier:15,medicalQuarantineAndDecontamination:9,diplomaticStateContinuityAndArchive:4,unknownOrIncompletelyCatalogued:9,mobileYardStationAndLargeModular:6},
    legacyCustodyCounts:{releasedOrAbandonedSalvage:251,disputedOrUnresolvedTitle:96,memorialOrEvidence:53,quarantineOrHazard:38,restrictedStrategicException:25},
    economicReality:'Most hulls are obsolete, already stripped of standardized high-value components, more expensive to repair than replace, incompatible with current infrastructure, legally awkward, hazardous, or simply too remote for bulk recycling to pay for the tow.',
    practicalSalvagePolicy:'Routine low-value scavenging is usually ignored. Distant authorities become interested when recovery involves strategic weapons, intact transit systems, identity or continuity archives, biological printers, active cognition, living survivors, or unusually valuable technology.',
    politicalReality:'The yard persists through mutual neglect. Several governments inherit claims or embarrassing histories here, while none wants the expense or diplomatic consequences of comprehensively reopening the site.',
    arnockConnection:{historicalCivilization:"Ar'nock Concord of Holds",contemporaryPolity:'Ossian Mandate',sectorSpeciesRecord:'Ossian Assemblies',relationship:'The Ar\'nock remain an extant biological people. Ossian is the postwar political and civilizational identity used by the fixed sector archive, not a replacement species.'}
  }];
  ...
  roots.forEach((root,index)=>{
    const disposition=dispositions[index%dispositions.length],generatedBiology=biologies[index*3%biologies.length],technology=technologies[(index*2+1)%technologies.length],empireScale=scales[(index*3+1)%scales.length],government=governments[index%governments.length];
    ...
    const generatedLifespanYears=integer(35,620);
    const generatedReproduction=pick(['sexual pair-bonded','communal spawning','budding caste lines','clonal broods','fabricated successors','seasonal polyparental']);
    const generatedSpecialties=[pick(specialties),pick(specialties),pick(specialties)];
    const generatedBestiaryTags=[pick(['megafauna','aerial predators','subsurface colonies','vacuum-adapted symbiotes']),pick(['engineered war organisms','planktonic intellects','crystalline parasites','machine fauna'])];
    const isArNockSuccessor=root==='Ossian';
    const biology=isArNockSuccessor?{metabolism:'oxygen-carbon engineered endotherm using copper-bearing respiratory metalloproteins',bodyPlan:'tall narrow six-limbed segmented upright tool user',nativeGravity:'0.75–0.85 g',nativeEnvironment:'warm dry oxygenated environment with acidic and unfamiliar trace compounds',lifespanYears:150,reproduction:'sexual small-clutch reproduction with brood-cohort rearing'}:{metabolism:generatedBiology[0],bodyPlan:generatedBiology[1],nativeGravity:generatedBiology[2],nativeEnvironment:generatedBiology[3],lifespanYears:generatedLifespanYears,reproduction:generatedReproduction};
    species.push({speciesId,name,status:'extant',polityId,homeClusterId,dispositionArchetype:disposition[0],sectorStance:disposition[1],behavioralSummary:disposition[2],biology,technology:{principalBand:technology[0],transit:technology[1],energy:technology[2],inertialControl:technology[3],combat:technology[4],specialties:generatedSpecialties},bestiaryTags:generatedBestiaryTags,controlledClusterIds,...(isArNockSuccessor?{biologicalIdentity:"Ar'nock",historicalCivilization:"Ar'nock Concord of Holds",currentCivilizationalIdentity:'Ossian Assemblies',successorPolityName:'Ossian Mandate',successionNote:"The Ar'nock remain an extant people. After the Boundary Wars, Hold Secession conflicts, and the Ossian constitutional settlement, their interstellar state was reorganized and is now catalogued under the Ossian political identity.",historicalTransitNote:'The modern Ossian Mandate uses its current generated transit technology; that does not identify the FTL family installed on ancient pre-Ossian Ar\'nock vessels such as the No Return Signal derelict.'}:{})});
    const controlledPlanetCount=integer(2,18)*controlledClusterIds.length;
    const populationBillions=number(.4,950)*controlledClusterIds.length;
    const lawAndCulture=[pick(['formal treaty law','honor arbitration','corporate charter law','ecological personhood','machine consensus protocols']),pick(['hereditary privilege','military emergency codes','open scientific commons','ritual senatorial debate'])];
    const strategicObjectives=[pick(['secure transit corridors','recover precursor archives','contain hostile expansion','open commercial concessions','preserve biospheres']),pick(['reverse demographic decline','expand buffer zones','isolate dangerous ruins','maintain senate coalition','monopolize Q-condensate'])];
    polities.push({polityId,name:polityName,speciesIds:[speciesId],government,empireScale,capitalClusterId:homeClusterId,controlledClusterIds,controlledPlanetCount,populationBillions,diplomaticPosture:disposition[0],sectorStance:disposition[1],lawAndCulture,strategicObjectives,...(isArNockSuccessor?{historicalNames:["Ar'nock Concord of Holds"],constitutionalLineage:'Postwar successor state retaining Ar\'nock population, continuity jurisprudence, custodial institutions, biological-fabrication traditions, and much of the old Hold legal inheritance.',nameHistory:'Ossian is a modern political and civilizational designation arising from the postwar constitutional settlement; it does not denote replacement or extinction of the Ar\'nock species.'}:{})});
