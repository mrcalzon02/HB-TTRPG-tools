(() => {
  ...
  function validate(record){
    ...
    if(geometry){
      ...
      if(geometry.observerCombatAuthority?.lateralCombatAccelerationMps2>(record.propulsionLedger?.lateralCombatAccelerationMps2||0)+1e-9)violations.push('VESSEL-06 observer acceleration exceeds its engineering authority.');
      if(geometry.observerCombatAuthority?.combatReserveDeltaVMps>(record.propulsionLedger?.combatReserveDeltaVMps||0)+1e-9)violations.push('VESSEL-06 observer combat delta-v exceeds its engineering authority.');
