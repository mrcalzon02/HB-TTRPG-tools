(() => {
  ...
  function render(vessel){
    ...
    const repairs=$('exo-vessel-condition-repair-log'),repairRows=history.repairLog.length?history.repairLog.map(repair=>node('li','',`${repair.type}: ${repair.description}`)):[node('li','','No repairs were required.')];if(repairs)repairs.replaceChildren(...repairRows);
  }
