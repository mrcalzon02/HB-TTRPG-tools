(() => {
  ...
  const ASSET_VERSION = '20260809-cubic-decryptor-hardening-2';
  ...
  function openSignalsLaboratory(button = null, options = null) { return withLoadingButton(button, 'Loading Signals Laboratory…', async () => { const api = await loadSignalsLaboratory(); if (!api?.openPanel) throw new Error('The Signals Laboratory loaded without an open-panel interface.'); return api.openPanel(options || {}); }); }
  ...
  function buildWorkspace() {
    ...
    view.innerHTML = `
      ...
      <section class="scientific-tools-panel no-print" data-scientific-tools-panel="signals-laboratory" hidden><p class="eyebrow">Radio-frequency and electromagnetic measurement research</p><h3>Signals Laboratory</h3><p>Explore antenna attenuation and tuning, RLC/feed impedance, free-space path loss, thermal noise, heterodyne translation, nonlinear intermodulation, known-carrier perturbation, beat/sideband signatures, and algebraic frequency/range candidate inference in one setting-neutral electromagnetic workbench.</p><div class="scientific-tools-actions"><button id="scientific-tools-open-signals-laboratory" type="button" class="primary-action">Open Signals Laboratory</button></div><div class="scientific-tools-runtime"><span><strong>Direct field model:</strong> wavelength, free-space loss, field strength, receiver noise floor, antenna response and modeled SNR</span><span><strong>Antenna model:</strong> series-RLC impedance, feed mismatch, resonant attenuation/tuning sweep and log-frequency visualization</span><span><strong>Heterodyne model:</strong> sum/difference translation around a local oscillator with image candidates retained</span><span><strong>Indirect detection experiments:</strong> adjacent-carrier beats, sideband perturbation and second/third-order intermodulation products are tested only when a coupling/nonlinearity mechanism is supplied</span><span><strong>Inference:</strong> candidate frequencies and range scenarios preserve ambiguous solutions and expose their assumptions instead of claiming a unique source</span></div><div class="scientific-tools-boundary"><strong>Physical boundary:</strong> signal processing can exploit a weak field or correlated perturbation that reaches the receiver, but it cannot recover arbitrary RF energy that creates no measurable coupling, beat, loading, leakage, sideband, impedance change, or nonlinear product at the antenna/front end. This laboratory keeps direct measurement, indirect signatures, and inference visibly separate.</div></section>
