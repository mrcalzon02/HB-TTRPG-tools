(() => {
  ...
  function inferArchetype(input,result,source) {
    ...
    const dossier = sourceDossier(source);
    const species = dossier?.species || {};
    const civilization = dossier?.civilization || {};
    const system = dossier?.system || {};
    const role = result.identity?.roleKey || input.role || 'explorer';
  ...
  function resolveBias(seed,input,result,source) {
    const inferred = inferArchetype(input,result,source);
  ...
  const designationPrefix = key => ({VAULT_KEEPER:'VK',VOID_NOMAD:'VN',CORP_LOGISTICS:'CL',APEX_WARLORD:'AW'})[key] || 'EX';
  ...
  function geometry(volumeM3,shape) {
    const ratioH=.72;
    const beam=Math.cbrt(Math.max(1,volumeM3)/(Math.PI/6*Math.max(2,shape)*ratioH));
  ...
  function apply(seed,input,source,result) {
    const bias=resolveBias(seed,input,result,source);
    const baseline=clone(result.hull.massBudget || []);
    const assignments=baseline.map(row=>{
      ...
      const probability=moduleProbability(row.key,bias);
      const envelope=unit(`${seed}:module:${row.key}`)<probability?'INTERNAL':'EVA';
      const philosophy=philosophyMap[envelope];
      const floor=Math.min(finite(row.massTonnes),Math.max(0,inventoryFloor(row,result)));
      const affected=Math.max(0,finite(row.massTonnes)-floor);
      const adjustedMass=floor+affected*philosophy.massMultiplier;
      const adjustedVolume=finite(row.volumeM3)*philosophy.volumeMultiplier;
      const parentId=envelope==='INTERNAL'?`pressure-vault-${row.key}`:`vacuum-truss-${row.key}`;
      return {
        ...row,
        baselineMassTonnes:finite(row.massTonnes),baselineVolumeM3:finite(row.volumeM3),physicalInventoryFloorTonnes:floor,architectureAffectedMassTonnes:affected,
        massTonnes:adjustedMass,volumeM3:adjustedVolume,
        moduleType:module.label,envelope,concreteForm:envelope==='INTERNAL'?module.internalForm:module.evaForm,germinationTag:envelope==='INTERNAL'?module.internalTag:module.evaTag,
        attachment:{parentId,requiredProperty:philosophy.attachmentProperty,parentProperties:[philosophy.attachmentProperty,envelope==='INTERNAL'?'PRESSURIZED_ACCESS':'REMOTE_SERVICEABLE'],valid:true},
        crewDependent:module.crewDependent,utilityRouting:philosophy.utilityRouting,structuralEnvelope:philosophy.structuralEnvelope,
        modifiers:{massMultiplier:philosophy.massMultiplier,volumeMultiplier:philosophy.volumeMultiplier,armorEfficiency:philosophy.armorEfficiency,repairTimeMultiplier:philosophy.repairTimeMultiplier,cascadeRisk:philosophy.cascadeRisk,cascadeRiskIndex:philosophy.cascadeRiskIndex,thermalSignatureMultiplier:philosophy.thermalSignatureMultiplier},
        repairTimeIndex:philosophy.repairTimeMultiplier,thermalSignatureIndex:philosophy.thermalSignatureMultiplier,effectiveArmorIndex:philosophy.armorEfficiency,
        note:`${row.note} Architectural form: ${envelope==='INTERNAL'?module.internalForm:module.evaForm}; ${envelope==='INTERNAL'?module.internalTag:module.evaTag}.`
      };
    });

    const totalMass=assignments.reduce((sum,row)=>sum+row.massTonnes,0);
    const totalVolume=assignments.reduce((sum,row)=>sum+row.volumeM3,0);
    const internalVolume=assignments.filter(row=>row.envelope==='INTERNAL').reduce((sum,row)=>sum+row.volumeM3,0);
    const internalMass=assignments.filter(row=>row.envelope==='INTERNAL').reduce((sum,row)=>sum+row.massTonnes,0);
    const internalRatio=internalVolume/Math.max(1e-12,totalVolume);
    const classification=classify(internalRatio);
    const weighted=field=>assignments.reduce((sum,row)=>sum+row.massTonnes*finite(row.modifiers[field],1),0)/Math.max(1e-12,totalMass);
    const repairMultiplier=weighted('repairTimeMultiplier');
    const cascadeIndex=weighted('cascadeRiskIndex');
    const thermalSignature=weighted('thermalSignatureMultiplier');
    const armorEfficiency=weighted('armorEfficiency');
    const role=roleMap[result.identity?.roleKey] || roleMap.explorer || {shape:5};
    const shape=classification==='INTERNAL'?Math.max(3.2,role.shape*.84):classification==='EVA'?role.shape*1.28:role.shape;
    const hullGeometry=geometry(totalVolume,shape);
    const baseMass=baseline.reduce((sum,row)=>sum+finite(row.massTonnes),0);
    const baseVolume=baseline.reduce((sum,row)=>sum+finite(row.volumeM3),0);
    const massFactor=totalMass/Math.max(1e-12,baseMass);
    const volumeFactor=totalVolume/Math.max(1e-12,baseVolume);
    const maintainability=clamp(100*(1/repairMultiplier)*(.72+bias.profile.standardization*.28),10,100);
    const productionCostFactor=massFactor*(.72+volumeFactor*.28)*(1.12-bias.profile.standardization*.18);
    const code=`${designationPrefix(bias.profile.key)}-${classification==='INTERNAL'?'INT':classification==='EVA'?'EVA':'HYB'}-${Math.round(internalRatio*100).toString().padStart(2,'0')}`;
    const dossier=sourceDossier(source);
    const originSpecies=dossier?.species?.name || result.lifeSupport?.profile?.sourceSpecies || 'unidentified originating species';
    const originOrganization=dossier?.civilization?.government || dossier?.civilization?.economy || bias.profile.label;
    ...
    result.hull.baselineMassTonnes=baseMass;
    result.hull.baselineVolumeM3=baseVolume;
    result.hull.totalMassTonnes=totalMass;
    result.hull.totalVolumeM3=totalVolume;
    result.hull.lengthM=hullGeometry.lengthM;result.hull.beamM=hullGeometry.beamM;result.hull.heightM=hullGeometry.heightM;result.hull.decks=hullGeometry.decks;result.hull.surfaceAreaM2=hullGeometry.surfaceAreaM2;
    result.hull.averageDensityTonnesM3=totalMass/Math.max(1e-12,totalVolume);
    result.hull.massBalanceErrorTonnes=totalMass-assignments.reduce((sum,row)=>sum+row.massTonnes,0);
    result.hull.massBudget=assignments.map(row=>({...row,massPercent:row.massTonnes/totalMass*100,massText:formatMass(row.massTonnes),volumeText:`${fmt(row.volumeM3,1)} m³`}));
    result.hull.totalMassText=formatMass(totalMass);
    result.hull.dryMassTonnes=totalMass-finite(result.fuel?.carriedFuelTonnes);
