(function installLargeGridBinaryCubeEngine(root, factory) {
  'use strict';
  const api = factory();
  if (typeof module === 'object' && module.exports) module.exports = api;
  if (root) root.ShadowrunBinaryCubeEngine = api;
})(typeof globalThis !== 'undefined' ? globalThis : this, function createLargeGridBinaryCubeEngine() {
  'use strict';
  ...
  function projectionOrder(key, face, quarterTurns = 0) {
    if (!FACES.includes(face)) fail(`Unknown cube face: ${face}`);
    const size = key.gridSize;
  ...
  function projectionDiagnostics(rawKey) {
    ...
    for (const face of FACES) {
      const order = projectionOrder(key, face, 0);
      let valid = true;
  ...
  function transformBlockWithKey(block, key, fromFace, fromTurns, toFace, toTurns) {
    ...
    const inputOrder = projectionOrder(key, fromFace, fromTurns);
    const outputOrder = projectionOrder(key, toFace, toTurns);
