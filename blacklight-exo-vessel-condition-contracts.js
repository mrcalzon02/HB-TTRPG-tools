(() => {
  ...
  function validate(record){
    ...
    if(history){
      ...
      for(const module of modules){const state=states.find(item=>item.moduleId===module.moduleId);if(state){const{moduleId,...expected}=state;if(JSON.stringify(module.state)!==JSON.stringify(expected))violations.push(`${module.moduleId} state diverges from canonical condition history.`);}}
      ...
      if(history.axes?.destructionPercent===100&&(history.coherentVesselGraph||Object.values(history.effectiveGraphs||{}).some(graph=>graph.nodes?.length||graph.edges?.length)))violations.push('Total destruction retained coherent effective topology.');
