(() => {
  ...
  async function verifySolar(){let frame;try{frame=await timeout(frameLoad('blacklight-exo-solar-system.html'),10000,'Solar frame load');const evidence=await pollFrame(frame,(win,doc)=>{const system=win.BlacklightExoGetActiveSystem?.(),name=doc.getElementById('exo-summary-name')?.textContent,status=doc.getElementById('exo-cluster-status')?.textContent;if(system?.planets?.length&&name&&name!=='pending')return{system,name,status};return null;},'Solar lightweight runtime');const diagnostics=supervisorReport(frame.contentWindow),criticalFailures=(diagnostics.failures||[]).filter(item=>criticalSolar(item.phase));return{ok:criticalFailures.length===0,name:evidence.name,planets:evidence.system.planets.length,status:evidence.status,criticalFailures,diagnostics};}finally{frame?.remove();}}
  async function verifySector(){let frame;try{frame=await timeout(frameLoad('blacklight-exo-stellar-sector.html'),10000,'Sector frame load');const evidence=await pollFrame(frame,(win,doc)=>{const sector=win.BlacklightExoGetActiveSector?.(),name=doc.getElementById('exo-sector-summary-name')?.textContent,status=doc.getElementById('exo-sector-status')?.textContent;if(sector?.clusters?.length>=24&&name&&name!=='loading')return{sector,name,status};return null;},'Stellar Sector runtime');const diagnostics=supervisorReport(frame.contentWindow),criticalFailures=(diagnostics.failures||[]).filter(item=>criticalSector(item.phase));return{ok:criticalFailures.length===0,name:evidence.name,clusters:evidence.sector.clusters.length,species:evidence.sector.species.length,worlds:evidence.sector.worlds?.length||0,status:evidence.status,criticalFailures,diagnostics};}finally{frame?.remove();}}
