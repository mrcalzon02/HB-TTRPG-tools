(() => {
  ...
  function measureCurrentSystem() {
    ...
    const totalSolarMass = stellarMassSolar + planetaryMassSolar + beltMassSolar;
    const record = {
      seed,
      stellarMassSolar,
      planetMassEarth,
  ...
  function decorateCards() {
    for (const card of document.querySelectorAll('#exo-cluster-grid .exo-cluster-card')) {
      ...
      if (!record) continue;
      card.dataset.stellarMass = String(record.stellarMassSolar);
      card.dataset.planetaryMass = String(record.planetaryMassSolar + record.beltMassSolar);
      ...
      if (dd) dd.textContent = `${formatSolarMass(record.totalSolarMass)} M☉`;
      wrapper.title = `${record.planetMassEarth.toFixed(3)} Earth masses in planets; ${record.moonMassEarth.toFixed(3)} Earth masses in moons; ${record.beltMassLunar.toFixed(3)} lunar masses in belts.`;
    }
  }
