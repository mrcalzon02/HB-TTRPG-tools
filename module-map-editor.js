(() => {
  ...
  function init(){
    ...
    root.innerHTML=`
      <div class="section-heading">
        ...
        <h2>Tile-Based Module Map Editor</h2>
        <p>Build a clean grid map directly: cycle black/white/door/secret/trap/stairs/label tiles, adjust map dimensions, and export JSON or SVG for later module import.</p>
      </div>
      <div class="module-editor-layout">
        <aside class="module-editor-tools">
          ...
          <div class="module-editor-actions"><button id="mme-apply-size" type="button">Apply Size</button><button id="mme-clear" type="button">Clear</button><button id="mme-export-json" type="button">Export JSON</button><button id="mme-export-svg" type="button">Export SVG</button><button id="mme-copy-ascii" type="button">Copy ASCII</button></div>
          <label class="control-label">Import JSON<textarea id="mme-import" placeholder="Paste exported map JSON here"></textarea></label>
          <div class="module-editor-actions"><button id="mme-import-json" type="button">Import JSON</button></div>
          <p class="module-editor-status" id="mme-status">39 × 39 grid ready.</p>
          <pre class="module-editor-preview" id="mme-preview"></pre>
        </aside>
        <div class="module-editor-map-shell"><div id="mme-grid" class="module-tile-grid" aria-label="Tile map editor"></div></div>
      </div>`;
  ...
  function bind(root){
    ...
    root.querySelector('#mme-apply-size').onclick=()=>{ const w=clampInt(root.querySelector('#mme-width').value,1,120); const h=clampInt(root.querySelector('#mme-height').value,1,120); resizeMap(w,h); render(root); };
    ...
    root.querySelector('#mme-import-json').onclick=()=>{ try{ const parsed=JSON.parse(root.querySelector('#mme-import').value); if(!parsed.width||!parsed.height||!Array.isArray(parsed.cells)) throw new Error('Invalid map JSON'); state=parsed; root.querySelector('#mme-width').value=state.width; root.querySelector('#mme-height').value=state.height; render(root); status(root,'Imported map JSON.'); }catch(e){ status(root,'Import failed: '+e.message); }};
