(() => {
  ...
  function build(result,input){
    const mode=validationMode(input),scenario=C.scenario(input),observer=C.observer(result),seed=`${result.contract.seeds.equipmentSeed}:track:${result.contract.seeds.historySeed}`,sensorSources=C.sources(result,observer,seed),track=C.buildTrack(result,observer,scenario,seed);
  ...
  function apply(input,result){
    ...
    if(result.sensorLedger)result.sensorLedger.trackModelStatus=`VESSEL-06 generated ${record.tracks.length} target track with ${record.sensorSources.length} surviving sensor sources; weapon-family engagement remains deferred to VESSEL-07.`;
