(function installBinaryCubeDiagnosticPipeline(root, factory) {
  ...
  function resolveCalibrationSnapshot(options, deps) {
    return options?.calibrationSnapshot || deps?.calibrationBaseline?.snapshot || deps?.calibration?.defaultSnapshot || null;
  }

  function compactCalibration(value) {
    if (!value) return null;
    return Object.freeze({ detectorId: value.detectorId, cases: value.matrix?.cases || 0, sensitivity: value.sensitivity, specificity: value.specificity, balancedAccuracy: value.balancedAccuracy, shrinkage: value.shrinkage || 0, effectiveReliability: value.effectiveReliability, effectiveWeight: value.effectiveWeight, calibrationStatus: value.calibrationStatus || 'prior-only' });
  }
  ...
  function bitOneFraction(bytesValue) {
    ...
    if (!bytes.length) return 0;
    let ones = 0;
    for (const byte of bytes) { let value = byte; while (value) { ones += value & 1; value >>>= 1; } }
  ...
  function uniqueByteFraction(bytesValue) {
    const bytes = asBytes(bytesValue);
    if (!bytes.length) return 0;
    const seen = new Uint8Array(256); let unique = 0;
    for (const byte of bytes) if (!seen[byte]) { seen[byte] = 1; unique += 1; }
    return unique / 256;
  }

  function parseKnownArtifact(bytesValue) {
    const bytes = asBytes(bytesValue); const text = decodeText(bytes).trim();
  ...
  function classifyAsset(bytesValue, hints = {}) {
    ...
    if (magic) {
      let subtype = magic.id; let label = magic.label; let mime = magic.mime;
  ...
  function applicability(definition, classification, options, deps) {
    ...
    switch (definition.id) {
      ...
      case 'audio-signal-forensics': {
        const utils = deps.media?.utilities;
  ...
  function buildPlan(bytesValue, optionsValue = {}) {
    ...
    const snapshot = resolveCalibrationSnapshot(options, deps);
    const detectors = DETECTOR_DEFINITIONS.map(definition => {
      const app = applicability(definition, classification, options, deps);
      const calibration = compactCalibration(deps.calibration?.calibrationFor?.(snapshot, definition.id));
      const weight = app.applicable ? (deps.calibration?.effectiveWeight?.(definition.id, app.weight, snapshot) ?? app.weight) : 0;
      return Object.freeze({ ...definition, ...app, weight, calibration });
    });
  ...
  function finding(definition, values = {}) {
    const rawReliability = clamp(values.reliability == null ? 0.7 : values.reliability);
    const calibration = definition.calibration || null;
    const influence = calibration && calibration.calibrationStatus !== 'prior-only' ? clamp((calibration.shrinkage || 0) * 0.75) : 0;
    const reliability = clamp(rawReliability * (1 - influence) + (calibration?.effectiveReliability ?? rawReliability) * influence);
    return Object.freeze({
      ...
      calibrationCases: calibration?.cases || 0,
  ...
  async function executeDetector(definition, context) {
    ...
    if (definition.id === 'media-forensic-sweep') {
      ...
      const container = report.bytes?.container || {}; const candidates = report.bytes?.candidates || [];
    ...
    if (definition.id === 'png-structure') {
      const report = deps.steganalysis.inspectPngMetadata(bytes); const textChunks = report.textChunks?.length || 0; const trailing = report.trailingBytes || 0;
    ...
    if (definition.id === 'jpeg-coefficients') {
      const metadata = deps.steganalysis.inspectJpegMetadata(bytes); const coefficients = deps.steganalysis.inspectJpegCoefficients(bytes);
      const supported = coefficients.supported === true;
      const imbalance = supported ? Math.max(0, ...coefficients.components.map(component => Number(component.oddEvenImbalance || 0))) : 0;
      const positive = supported ? clamp((0.08 - imbalance) / 0.08) * 0.2 : 0;
      return finding(definition, { status: supported ? (positive > 0.15 ? 'mixed' : 'informational') : 'inconclusive', positiveEvidence: positive, reliability: supported ? 0.55 : 0.2, sampleSufficiency: supported ? sampleSufficiency(coefficients.components.reduce((sum, component) => sum + (component.blocks || 0), 0), 2048) : 0.2, metrics: { metadataSegmentCount: metadata.segments?.length || 0, coefficientDecodeSupported: supported, reason: coefficients.reason || null, componentCount: coefficients.components?.length || 0, maximumOddEvenImbalance: imbalance }, notes: [coefficients.caveat || coefficients.reason || 'JPEG coefficient statistics are a forensic feature set, not a specific embedding verdict.'], sensitivity: ['jpeg-metadata','baseline-dct-coefficients','odd-even-coefficient-populations'], raw: { metadata, coefficients } });
    }
    if (definition.id === 'audio-signal-forensics') {
      const utils = deps.media.utilities;
      const wav = utils.parseWav(bytes);
      if (!wav?.valid) return finding(definition, { status: 'inconclusive', reliability: 0.2, sampleSufficiency: 0, notes: ['Audio classification matched, but the routed decoder currently requires a supported RIFF/WAVE stream.'], sensitivity: ['dtmf','binary-fsk','afsk-1200-2200'] });
      let decoded;
      try { decoded = utils.decodeWavChannels(bytes, wav, { maxFrames: Math.min(wav.frameCount, wav.sampleRate * 30) }); }
      catch (error) { return finding(definition, { status: 'inconclusive', reliability: 0.25, sampleSufficiency: 0.15, metrics: { wavFormat: wav.audioFormat, bitsPerSample: wav.bitsPerSample }, notes: [`WAVE signal decode could not execute: ${error.message}`], sensitivity: ['dtmf','binary-fsk','afsk-1200-2200'] }); }
      const samples = decoded.channels?.[0] || [];
      const dtmf = utils.decodeDtmf(samples, wav.sampleRate);
      const afsk = utils.decodeBinaryFsk(samples, wav.sampleRate, { markFrequency: 1200, spaceFrequency: 2200, baud: 1200 });
      const spectrum = utils.spectralSummary ? utils.spectralSummary(samples, wav.sampleRate) : null;
      const dtmfKeys = String(dtmf?.keys || '');
      const dtmfEvidence = dtmfKeys.length >= 3 ? clamp(0.52 + Math.min(0.4, dtmfKeys.length * 0.04)) : dtmfKeys.length ? 0.18 : 0;
      const afskBytes = afsk?.bytesMsb || new Uint8Array(0);
      const afskPrintable = printableFraction(afskBytes);
      const afskConfidence = clamp(afsk?.meanConfidence || 0);
      const afskEvidence = afskBytes.length >= 2 ? clamp(afskConfidence * 0.55 + Math.max(0, afskPrintable - 0.45) * 0.65) : 0;
      const positive = Math.max(dtmfEvidence, afskEvidence);
      const status = positive >= 0.55 ? 'positive' : positive >= 0.3 ? 'mixed' : 'negative';
      return finding(definition, { status, positiveEvidence: positive, negativeEvidence: positive < 0.25 ? 0.3 : 0.03, reliability: 0.74, sampleSufficiency: sampleSufficiency(samples.length, Math.max(2048, wav.sampleRate)), metrics: { sampleRate: wav.sampleRate, channels: wav.channels, durationSeconds: wav.durationSeconds, dtmfKeys, dtmfFrameCount: dtmf?.frames?.length || 0, afskBitCount: afsk?.bits?.length || 0, afskMeanConfidence: afskConfidence, afskPrintableFraction: afskPrintable, afskPreview: decodeText(afskBytes.slice ? afskBytes.slice(0, 96) : afskBytes), dominantFrequencies: spectrum?.dominant?.slice(0,6) || [] }, notes: ['Automatic audio routing tests existing DTMF and common 1200/2200 Hz binary-FSK tools. Other modulation families remain unresolved and contribute to miss-risk rather than being declared absent.'], sensitivity: ['dtmf','binary-fsk','afsk-1200-2200','spectral-carriers'] });
    }
    ...
    if (definition.id === 'binary-cube-attack-suite') {
      ...
      const best = results[0] || null; const positive = best ? clamp((Number(best.score || 0) - 55) / 45) : 0;
  ...
  function aggregateEvidence(plan, findings, errors = []) {
    ...
    const calibrationCases = findings.reduce((sum, item) => sum + (item.calibrationCases || 0), 0);
    const calibratedFindings = findings.filter(item => item.calibrationStatus !== 'prior-only').length;
    const calibrationIndex = planned.length ? clamp(calibratedFindings / planned.length) : 0;
    const certaintyIndex = clamp(coverageIndex * 0.34 + sampleIndex * 0.2 + conclusionAgreement * 0.2 + independenceIndex * 0.16 + calibrationIndex * 0.1);
  ...
  async function runConcurrent(taskItems, concurrency, worker) {
    const results = new Array(taskItems.length); let cursor = 0;
    const count = Math.max(1, Math.min(taskItems.length || 1, concurrency || 1));
    const runners = Array.from({ length: count }, async () => { while (true) { const index = cursor++; if (index >= taskItems.length) return; results[index] = await worker(taskItems[index], index); } });
    await Promise.all(runners); return results;
  }

  async function runPipeline(bytesValue, optionsValue = {}) {
    const bytes = asBytes(bytesValue); const sourceName = String(optionsValue.sourceName || 'diagnostic-input');
    const profile = normalizeProfile(optionsValue.profile); const options = { ...optionsValue, profile };
    const classification = classifyAsset(bytes, options); const deps = dependencies();
    const calibrationSnapshot = resolveCalibrationSnapshot(options, deps);
    const plan = buildPlan(bytes, { ...options, classification, calibrationSnapshot });
    ...
    const findings = []; const errors = []; const applicable = plan.detectors.filter(item => item.applicable);
    const totalWeight = applicable.reduce((sum, item) => sum + item.weight, 0) || 1; let completedWeight = 0;
    ...
    for (const stage of plan.stages) {
      ...
      const stageResults = await runConcurrent(stageItems, profile.concurrency, async definition => {
        ...
        try { return { definition, result: await executeDetector(definition, context), error: null }; }
        catch (error) { if (token.cancelled || error?.name === 'AbortError') throw error; return { definition, result: null, error }; }
      });
      for (const row of stageResults) {
        completedWeight += row.definition.weight;
        if (row.error) { errors.push(Object.freeze({ detectorId: row.definition.id, name: row.error.name || 'Error', message: row.error.message || String(row.error) })); findings.push(finding(row.definition, { status: 'error', reliability: 0, sampleSufficiency: 0, notes: [row.error.message || String(row.error)] })); }
    ...
    return Object.freeze({
      format: REPORT_FORMAT, schemaVersion: REPORT_SCHEMA_VERSION, pipelineVersion: VERSION,
      ...
      calibration: Object.freeze({ registryVersion: deps.calibration?.version || null, snapshotFormat: calibrationSnapshot?.format || null, corpusVersion: calibrationSnapshot?.corpusVersion || 'none', receiptCount: calibrationSnapshot?.receiptCount || 0, boundary: calibrationSnapshot?.boundary || 'No empirical calibration snapshot was available; declared detector priors were used.' }),
      classification, plan, indices, findings: Object.freeze(findings), errors: Object.freeze(errors), completed: true
    });
  }
