...
const finite=(value,label)=>{if(!Number.isFinite(value))fail(`${label} is not finite.`);return value;};
const finiteRange=(value,label)=>{finite(value?.minimum,`${label}.minimum`);finite(value?.maximum,`${label}.maximum`);if(value.minimum>value.maximum)fail(`${label} minimum exceeds maximum.`);};
...
for(const item of matrix.cases){if(item.counts?.modules!==item.counts?.placements)fail(`${item.caseId} module and placement totals diverge.`);for(const key of ['massLedgerErrorTonnes','moduleMassErrorTonnes','moduleVolumeErrorM3'])finite(item.closure?.[key],`${item.caseId}.closure.${key}`);if(Math.abs(item.closure.massLedgerErrorTonnes)>Math.max(1,finite(item.identity?.totalMassTonnes,`${item.caseId}.identity.totalMassTonnes`))*1e-9)fail(`${item.caseId} mass ledger does not close.`);if(Object.values(item.statistics||{}).some(value=>!Number.isFinite(value)||value<0||value>100))fail(`${item.caseId} contains an invalid normalized statistic.`);for(const weapon of item.weapons||[])for(const key of ['physicalMaximumRangeM','practicalRangeM','practicalSolutionQualityPercent','effectiveRounds'])finite(weapon[key],`${item.caseId}/${weapon.family}.${key}`);}
for(const[key,value]of Object.entries(matrix.aggregate?.statisticRanges||{}))finiteRange(value,`matrix.statisticRanges.${key}`);for(const[key,value]of Object.entries(matrix.aggregate?.countRanges||{}))finiteRange(value,`matrix.countRanges.${key}`);finiteRange(matrix.aggregate?.availableActionRange,'matrix.availableActionRange');for(const[family,value]of Object.entries(matrix.aggregate?.weaponRangeExtrema||{})){finiteRange(value.physicalMaximumRangeM,`matrix.weaponRangeExtrema.${family}.physicalMaximumRangeM`);finiteRange(value.practicalRangeM,`matrix.weaponRangeExtrema.${family}.practicalRangeM`);finiteRange(value.solutionQualityPercent,`matrix.weaponRangeExtrema.${family}.solutionQualityPercent`);}
...
for(const state of engagement.states){if(state.trials!==64)fail(`${state.stateId} does not contain 64 trials.`);const total=Object.values(state.outcomes||{}).reduce((sum,value)=>sum+finite(value,`${state.stateId}.outcome`),0);if(total!==64)fail(`${state.stateId} outcome counts total ${total}, not 64.`);for(const key of ['meanImpactProbabilityPercent','observedImpactRatePercent','meanRetainedEnergyPercent','meanReadinessLossPercent']){finite(state[key],`${state.stateId}.${key}`);if(state[key]<0||state[key]>100)fail(`${state.stateId}/${key} is outside 0-100.`);}for(const key of ['meanResidualEnergyJ','meanDamagedModuleCount','meanDetachedModuleCount','meanSeveredRouteCount','meanCompromisedZoneCount','balanceScore']){finite(state[key],`${state.stateId}.${key}`);if(state[key]<0)fail(`${state.stateId}/${key} is negative.`);}}
const outcomeTotal=Object.entries(engagement.aggregate?.outcomeTotals||{}).reduce((sum,[key,value])=>sum+finite(value,`engagement.aggregate.outcomeTotals.${key}`),0);if(outcomeTotal!==9216)fail(`Aggregate engagement outcomes total ${outcomeTotal}, not 9,216.`);finiteRange(engagement.aggregate?.impactRateRange,'engagement.impactRateRange');finiteRange(engagement.aggregate?.readinessLossRange,'engagement.readinessLossRange');finite(engagement.aggregate?.comparableStateCount,'engagement.comparableStateCount');
