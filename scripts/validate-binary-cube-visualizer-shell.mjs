...
for (const gridSize of gridSizes) {
  const key = Engine.createKey({
    gridSize,
    seed: `visualizer-v4-shell-${gridSize}`,
    inputFace: 'top',
    outputFace: 'front',
    inputQuarterTurns: 0,
  ...
  assert.equal(new Set(normalized.map(position => position.join('|'))).size, points.length);
...
for (const [origin, direction, expectedFace] of [
  [[0,0,4],[0,0,-1],'front'], [[0,0,-4],[0,0,1],'back'], [[4,0,0],[-1,0,0],'right'],
  [[-4,0,0],[1,0,0],'left'], [[0,4,0],[0,-1,0],'top'], [[0,-4,0],[0,1,0],'bottom']
]) assert.equal(Renderer.rayBoxFace(origin, direction), expectedFace);
assert.equal(Renderer.rayBoxFace([4,4,4],[1,0,0]), null);
...
assert.match(rendererSource, /gl\.drawArrays\(mode,0,count\)/);
...
assert.match(controllerSource, /loaded canonical key remains unchanged/i);

for (const asset of ['binary-cube-visualizer.css','binary-cube-visualizer-renderer.js','shadowrun-binary-cube-visualizer.js']) assert.match(entrySource, new RegExp(asset.replaceAll('.', '\\.')));
...
for (const selector of ['cube-visualizer-canvas','cube-visualizer-label-layer','cube-visualizer-face-label','cube-visualizer-direction-label','cube-visualizer-fallback']) assert.match(styleSource, new RegExp(`\\.${selector}`));
