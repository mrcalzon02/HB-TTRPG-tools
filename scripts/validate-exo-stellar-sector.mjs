...
for(const file of['blacklight-exo-stellar-sector-data.js','blacklight-exo-stellar-sector-worlds.js','blacklight-exo-stellar-sector-generator.js','blacklight-exo-stellar-sector-contracts.js','blacklight-exo-stellar-sector.js','blacklight-exo-sector-archive-store.js'])if(!html.includes(file))fail(`Sector page does not load ${file}.`);
...
for(const signature of['indexedDB','localStorage','LEGACY_KEY','validateEnvelope','archiveHash','Import Sector Archive','Replay','Deterministic replay mismatch'])if(!archive.includes(signature))fail(`Durable sector archive layer lacks ${signature}.`);
...
if(manifest.schemaVersion!=='1.1.0'||!manifest.modules?.includes('blacklight-exo-stellar-sector-contracts.js')||!manifest.modules?.includes('blacklight-exo-sector-archive-store.js')||!manifest.archiveStorage?.includes('IndexedDB'))fail('Sector manifest is not pinned to the complete generator and durable archive stack.');
console.log('EXO stellar-sector generator, durable archive, and incremental solar loading validation passed.');