...
for (const marker of [
  'SCRIPT_TIMEOUT_MS = 4500', 'cdn.jsdelivr.net', 'unpkg.com',
  'Preparing the map only. No Chronicle data is being loaded.',
  "new CustomEvent('wod:spatial-map-ready'", 'Discover Named Locations'
]) {
  if (!mapCore.includes(marker)) throw new Error(`Map-only core is missing ${marker}.`);
}

for (const marker of [
  "COMPAT_VERSION = '2.7.0'", 'CORE_DATA_DEFAULTS', 'contextExpansion', 'detailDiversity',
  'wrapDetailCore', 'activeGameLine', 'activeGameLineStatusBridge: true',
  "freshUrl.searchParams.set('wod-config', COMPAT_VERSION)", "cache: 'no-store'",
  'syntheticConfigResponse', 'staleSchemaDefaultsApplied: true'
]) {
  if (!configCompat.includes(marker)) throw new Error(`Spatial config compatibility is missing ${marker}.`);
}

for (const marker of [
  "schemaVersion: '1.0.0'", 'siteTypes', 'hiddenFunctions', 'infrastructures', 'systemSecrets',
  'custodians', 'evidencePatterns', 'conflicts', 'consequences', 'cross-sphere-civic-junction',
  'feeding-permission-node', 'caern-catchment-site', 'fera-migration-waystation', 'hunter-safehouse',
  'freehold-annex', 'sanctum-annex'
]) {
  if (!siteCatalog.includes(marker)) throw new Error(`System site catalog is missing ${marker}.`);
}
for (const marker of [
  "VERSION = '3.1.0'", 'enhanceCore', 'regional-theme-local-product-v31',
  'compositional-product-space', 'legacyFrequencyDenominator', 'regionalThemeExpansionVersion'
]) {
  if (!regionalExpansion.includes(marker)) throw new Error(`Regional theme expansion is missing ${marker}.`);
}
for (const marker of [
  "VERSION = '1.0.0'", 'enhanceCore', 'systemSiteCatalogVersion', 'siteProfile',
  'Specific hidden function', 'Supernatural infrastructure', 'System secret', 'Evidence pattern',
  'Local struggle', 'Failure consequence'
]) {
...
const regionalExpansionIndex = loader.indexOf("'world-of-darkness-regional-theme-expansion.js'");
...
if ([configCompatIndex, siteCatalogIndex, diversityCoreIndex, regionalExpansionIndex, siteExpansionIndex, radialLoaderIndex].some(index => index < 0)) {
  throw new Error('One or more spatial core modules are absent from the loader.');
}
if (!(configCompatIndex < siteCatalogIndex
  && siteCatalogIndex < diversityCoreIndex
  && diversityCoreIndex < regionalExpansionIndex
  && regionalExpansionIndex < siteExpansionIndex
  && siteExpansionIndex < radialLoaderIndex)) {
  throw new Error('Regional and system-site expansions must load after the base core and before radial hydration.');
}

for (const marker of [
  'STATUS_PROFILES', 'standard: Object.freeze([12, 6, 2, 1])', 'unified: Object.freeze([5, 8, 5, 3])',
  'CATALOG_LINES', 'unified-catalog-line', 'catalogLine', 'createSession', 'neighborhoodKey',
  'inventoryStatusFromSeed', 'THEME_COMPONENTS', 'THEME_DYNAMICS', 'regionalThemeVariantCount',
  'themeVersion', 'diversitySignature'
]) {
  if (!diversityCore.includes(marker)) throw new Error(`Detail diversity core is missing ${marker}.`);
}
if (diversityCore.includes('fetch(') || diversityCore.includes('XMLHttpRequest')) throw new Error('The pure detail-diversity module must not perform network work during startup.');
if ([siteCatalog, regionalExpansion, siteExpansion].some(source => source.includes('fetch(') || source.includes('XMLHttpRequest'))) {
  throw new Error('Regional and system-site modules must remain pure and must not perform network work during startup.');
}

for (const marker of [
  'MAX_VISIBLE = 90', '.sort((left, right) => left.distance - right.distance', "map.on('movestart zoomstart'",
  'wod-radial-overall', 'wod:radial-location-ready', 'wod:radial-load-complete',
  'wod:radial-load-cancelled', 'return { ...payload, elements: [] }',
  'Selecting unused neighborhood details', 'detailDiversity'
]) {
...
if (radial.includes('characters_core.json') || radial.includes('rumors_core.json')) throw new Error('Radial generation returned to bundled character or rumor prototypes.');

for (const marker of ['wod:named-location-scan-complete', 'WODLightweightSpatialCore?.setLatestScan', "hydrationMode: 'radial-sequential'", 'wod-resolve-business']) {
  if (!compatibility.includes(marker)) throw new Error(`Compatibility bridge is missing ${marker}.`);
}
if (!loader.includes("document.addEventListener('wod:radial-load-complete'")) throw new Error('Advanced Chronicle tools do not wait for radial completion.');
...
console.log(JSON.stringify({
  ...
  pureRegionalAndSystemModuleNetworkRequests: 0,
