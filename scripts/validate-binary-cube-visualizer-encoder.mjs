...
assert.match(laboratorySource, /const Executor = window\.ShadowrunBinaryCubeWorkerClient/);
assert.match(laboratorySource, /runBackground\(panel, 'encrypt'/);
assert.match(laboratorySource, /runBackground\(panel, 'decrypt'/);
assert.equal(laboratorySource.includes('Engine.encryptBinary(panel.querySelector'), false, 'The laboratory must not bypass the freeze-safe worker for its primary encrypt path.');
assert.equal(laboratorySource.includes('Engine.decryptBinary(packageObject, key)'), false, 'The laboratory must not bypass the freeze-safe worker for its primary decrypt path.');
...
console.log(JSON.stringify({
  ...
  laboratoryWorkerDelegationPresent: true,
