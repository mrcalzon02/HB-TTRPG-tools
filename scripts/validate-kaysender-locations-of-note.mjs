...
if (!index.packs?.includes(packPath)) throw new Error('Kaysender wiki does not load the Mad Martiken Locations of Note pack.');
...
if (index.packs.at(-1) !== packPath) throw new Error('Mad Martiken location pack must load after the earlier rules and relic packs.');
...
if (!Array.isArray(entry.tables) || entry.tables.length !== 4) throw new Error('Mad Martiken entry must include four structured pricing and reference tables.');
...
const tableMap = new Map(entry.tables.map(table => [table.id,table]));
for (const id of ['martiken-advertised-services','martiken-expanded-pricing','martiken-trigger-reference','martiken-source-table-index']) {
...
const allText = [
  ...
  ...entry.tables.flatMap(table => [table.title,...table.columns,...table.rows.flat()]),
  ...(entry.tags || [])
].join(' ');
for (const phrase of [
...
const pricingText = sectionMap.get('Expanded service and surcharge price table').body.join(' ');
for (const amount of ['500 gp','1,000 gp','1,500 gp','2,000 gp','5,000 gp','12,000 gp','25,000 gp','35,000 gp']) if (!pricingText.includes(amount)) throw new Error(`Expanded pricing lacks '${amount}'.`);
...
if (receipt.integratedScope?.category !== 'Locations of Note' || receipt.integratedScope?.sourceMutationTables !== 6 || receipt.integratedScope?.sourceCaseStudies !== 2) throw new Error('Mad Martiken source receipt scope is incomplete.');

console.log('Kaysender Locations of Note validation passed.');
console.log('Verified Mad Martiken’s Menagerie, all fourteen source pages, three advertised service tiers, expanded 3.5 pricing, six source mutation tables, three trigger severities, two case files, and the proprietor stat block.');
