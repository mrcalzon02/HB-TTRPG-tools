(() => {
  ...
  function choose(seed,table){let roll=unit(seed);for(const [key,value] of Object.entries(table)){roll-=value;if(roll<=0)return key;}return'STANDARD';}
  function tech(seed,input,result){
    ...
    const principalRank=BANDS.indexOf(band)-1,profile=result.designation?.originArchetypeKey||'CORP_LOGISTICS',overrides=input.subsystemVariants||{};
    ...
    const subsystemVariants=(result.hull?.massBudget||[]).map(row=>{
      const forced=overrides[row.key]!=null,variant=forced?String(overrides[row.key]).toUpperCase():choose(`${seed}:technology:${row.key}`,weights(profile));
      const offset=VARIANTS[variant],heritageBand=band==='P0'&&variant==='LEGACY'&&LEGACY_OK.has(row.key)?'T-1':band;
      return{subsystemKey:row.key,label:row.label,principalBand:band,variant,offset,technologyIndex:principalRank+offset,heritageBand,source:forced?'explicit subsystem override':`${profile} provisional variant distribution`,rationale:heritageBand==='T-1'?`This P0 vessel explicitly uses sub-P0 heritage for ${row.label}.`:`${row.label} remains inside ${band} with a ${variant} implementation.`,explicitOverride:forced};
    });
  ...
  function seedRecord(seed,result,source){
    const d=dossier(source),species=d?.species?.name||result.designation?.originSpecies||'reference-species',org=d?.civilization?.government||result.designation?.originOrganization||'independent-organization',mfr=result.designation?.originArchetype||'provisional-manufacturer';
    const speciesSeed=`${d?.seed||seed}:species:${slug(species)}`,organizationSeed=`${speciesSeed}:organization:${slug(org)}`,manufacturerSeed=`${organizationSeed}:manufacturer:${slug(mfr)}`,hullFamilySeed=`${manufacturerSeed}:hull-family:${slug(result.identity?.roleKey)}:${slug(result.drive?.familyKey)}`,vesselInstanceSeed=String(seed);
  ...
  function ids(seeds,result,source){
    const d=dossier(source),species=d?.species?.name||result.designation?.originSpecies||'reference-species',org=d?.civilization?.government||result.designation?.originOrganization||'independent-organization',mfr=result.designation?.originArchetype||'provisional-manufacturer',hull=`${mfr}-${result.identity?.roleKey}-${result.drive?.familyKey}-${result.designPhilosophy?.classification}`;
    return{speciesId:id('species',species,seeds.speciesSeed),organizationId:id('org',org,seeds.organizationSeed),manufacturerId:id('mfr',mfr,seeds.manufacturerSeed),hullFamilyId:id('hull',hull,seeds.hullFamilySeed),vesselInstanceId:id('vessel',result.identity?.designationCode||result.identity?.name,seeds.vesselInstanceSeed)};
  }
  ...
  function validate(record){
    const c=record?.contract,violations=[];if(!c)violations.push('Missing contract envelope.');if(c?.schemaVersion!==SCHEMA)violations.push('Unexpected schema version.');
    ...
    if(!c?.technology?.validation?.valid)violations.push(...(c?.technology?.validation?.violations||[]));if(!c?.condition?.validation?.valid)violations.push(...(c?.condition?.validation?.violations||[]));
  ...
  function apply(seed,input,source,result){
    ...
    const sourceVersion=Number(result.version)||1,seeds=seedRecord(seed,result,source),identifiers=ids(seeds,result,source),technology=tech(seed,input,result),state=condition(input,result),time=result.generatedAt||new Date().toISOString();
    result.version=3;result.contract={recordType:'exoVessel',schemaVersion:SCHEMA,contractVersion:1,revision:1,createdAt:time,updatedAt:time,identifiers,seeds,technology,condition:state,derivedLayers:layers(),provenance:{generatorId:'blacklight-exo-vessel-generator',generatorVersion:'3.0.0',sourceRecordVersion:sourceVersion,sourceType:source?.type||result.source?.type||'standalone',registry:'data/exo-vessel/vessel-contract-registry.json',governingGuide:'EXO_VESSEL_SYSTEM_DESIGN_GUIDE.md',schema:'data/schemas/exo-vessel-record.schema.json',migrationRegistry:'data/exo-vessel/migrations.json'},migration:{policyVersion:SCHEMA,history:[{migrationId:`exo-vessel-v${sourceVersion}-to-contract-${SCHEMA}`,fromRecordVersion:sourceVersion,toRecordVersion:3,toSchemaVersion:SCHEMA,strategy:'append-only-envelope',preservedUnknownFields:true}]},validation:{valid:true,violations:[]},extensions:{}};
    result.contract.validation=validate(result);Object.assign(result.identity,identifiers,{technologyBand:technology.principalBand});result.condition=clone(state);
    result.warnings=[...(result.warnings||[]),`Canonical contract ${SCHEMA} assigns ${technology.principalBand} as the vessel-wide technology band; every subsystem remains within ±0.30.`,`Condition ${state.template} is schema-only until the condition and damage phase applies it to vessel graphs.`];return result;
  }
