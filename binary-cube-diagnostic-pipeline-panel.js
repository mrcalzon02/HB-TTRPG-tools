(() => {
  ...
  function renderFindings(report) {
    const cards = report.findings.map(item => {
      ...
      return `<article class="bcdp-finding" data-status="${esc(item.status)}"><header><strong>${esc(item.detectorId)}</strong><span>${esc(item.status)}</span></header><div class="bcdp-finding-metrics"><span>positive ${pct(item.positiveEvidence)}</span><span>negative ${pct(item.negativeEvidence)}</span><span>unresolved / miss-risk ${pct(item.missRiskEvidence)}</span><span>${esc(reliability)}</span><span>sample ${pct(item.sampleSufficiency)}</span><span>calibration ${esc(calibration)}</span></div><p>${item.notes.length ? esc(item.notes.join(' ')) : 'No additional note.'}</p><details><summary>Metrics</summary><pre>${esc(JSON.stringify(item.metrics, null, 2))}</pre></details></article>`;
    }).join('');
