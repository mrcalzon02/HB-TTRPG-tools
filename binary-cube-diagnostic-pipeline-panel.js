(() => {
  ...
  function ensureStyle() {
    ...
    link.rel = 'stylesheet';
    link.href = 'binary-cube-diagnostic-pipeline.css?v=20260809-diagnostic-pipeline-1';
  ...
  function renderPlan(report) {
    const rows = report.plan.detectors.map(item => `<tr><td>${item.stage}</td><td>${esc(item.id)}</td><td>${esc(item.cost)}</td><td>${item.applicable ? 'scheduled' : 'not applicable'}</td><td>${esc(item.reason)}</td></tr>`).join('');
    panel.querySelector('[data-bcdp-plan]').innerHTML = `<div class="bcdp-table"><table><thead><tr><th>Stage</th><th>Detector</th><th>Cost</th><th>Routing</th><th>Reason</th></tr></thead><tbody>${rows}</tbody></table></div>`;
  }

  function renderFindings(report) {
    const cards = report.findings.map(item => `<article class="bcdp-finding" data-status="${esc(item.status)}"><header><strong>${esc(item.detectorId)}</strong><span>${esc(item.status)}</span></header><div class="bcdp-finding-metrics"><span>positive ${pct(item.positiveEvidence)}</span><span>negative ${pct(item.negativeEvidence)}</span><span>reliability ${pct(item.reliability)}</span><span>sample ${pct(item.sampleSufficiency)}</span></div><p>${item.notes.length ? esc(item.notes.join(' ')) : 'No additional note.'}</p><details><summary>Metrics</summary><pre>${esc(JSON.stringify(item.metrics, null, 2))}</pre></details></article>`).join('');
    panel.querySelector('[data-bcdp-findings]').innerHTML = cards || '<p>No detector findings were produced.</p>';
  }

  function renderSummary(report) {
    const i = report.indices;
    panel.querySelector('[data-bcdp-summary]').innerHTML = `<div class="bcdp-index-grid"><article><span>Asset Presence Index</span><strong>${pct(i.presenceIndex)}</strong><p>Weighted positive evidence among completed applicable detectors.</p></article><article><span>Certainty Index</span><strong>${pct(i.certaintyIndex)}</strong><p>Coverage, sample sufficiency, agreement, and independent evidence groups.</p></article><article><span>Coverage Index</span><strong>${pct(i.coverageIndex)}</strong><p>How much of the routed detector plan actually completed.</p></article><article><span>Undetected / Miss-Risk Index</span><strong>${pct(i.missRiskIndex)}</strong><p>Rises when applicable methods could not run or opaque/random-like material remains unresolved.</p></article></div><div class="bcdp-verdict"><strong>${esc(i.classification.replaceAll('-', ' '))}</strong><p>${esc(i.boundary)}</p></div><p class="bcdp-classification"><strong>Asset classification:</strong> ${esc(report.classification.label)} · classifier confidence ${pct(report.classification.confidence)}</p>`;
  }
