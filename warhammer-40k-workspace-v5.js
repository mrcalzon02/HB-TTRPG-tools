(() => {
  ...
  const STYLE_PATH = 'warhammer-40k-workspace-v5.css?v=5';
  const UI_PATH = 'warhammer-40k-archive-ui-v4.js?v=4';
  const MAP_PATH = 'warhammer-40k-sector-map-v5.js?v=5';
  ...
  function buildMapPanel() {
    ...
    const navigation = el('section', 'wh-navigation-console');
    navigation.setAttribute('aria-label', 'Navis survey interaction controls');
    const navigationHead = el('div', 'wh-navigation-head');
    navigationHead.append(
      el('p', 'wh-kicker', 'Pinned helm controls'),
      el('span', 'wh-mode-readout', 'Grab / orbit mode · graduated resistance active')
    );
    const modebar = el('div', 'wh-modebar');
    modebar.setAttribute('role', 'toolbar');
    modebar.setAttribute('aria-label', 'Map interaction mode');
    const modeSpecs = [
      ['select', 'Select Contacts', 'Click contacts without moving the survey.'],
      ['orbit', 'Grab / Orbit', 'Drag slowly around the three-dimensional sector.'],
      ['pan', 'Pan Survey', 'Drag the chart laterally with damped resistance.'],
      ['zoom', 'Zoom Lens', 'Drag vertically, wheel, or pinch to zoom.']
    ];
    ...
    navigation.append(navigationHead, modebar);
    ...
    Object.entries(state.data.threatStates).forEach(([key, threat]) => {
      threatSelect.append(new Option(threat.label, key));
    });
    ...
    const layerSpecs = [
      ...
      ['routes', 'Transit and survey links', false],
      ['regions', 'Sector and campaign volumes', false],
      ['hazards', 'Threat volumes', false],
      ['labels', 'Canonical floating labels', true]
    ];
    ...
    stage.setAttribute(
      'aria-label',
      'Interactive three-dimensional Cafarron Corridor survey. Use the pinned Select, Grab and Orbit, Pan, or Zoom controls. Camera movement is deliberately damped and gradual.'
    );
    ...
    card.append(navigation, controls, stage);
    ...
    details.append(
      ...
      el('p', '', 'Select a system node to display its reference-sheet docket, source state, threat seal, and map relationships. Exploratory contact names remain concealed until selected.'),
      threatLegend()
    );
    ...
    function setMode(mode) {
      ...
      const readout = navigation.querySelector('.wh-mode-readout');
      const readouts = {
        select: 'Select mode · camera locked against accidental movement',
        orbit: 'Grab / orbit mode · graduated resistance active',
        pan: 'Pan mode · slow lateral chart movement active',
        zoom: 'Zoom lens mode · slow depth adjustment active'
      };
  ...
  function buildWorkspace(view) {
    ...
    copy.append(
      ...
      el('p', 'wh-note', 'Canonical system names are now assigned to the nearest left or right survey margin and connected by the shortest available leader line. Exploratory, unnamed, and provisional names remain sealed unless their contact is actively selected.'),
      el('p', 'wh-small', 'Camera movement uses deliberately heavy Navis damping. Select a helm mode from the pinned map console before manipulating the chart. White exploratory contacts remain non-canon and replacement-ready.')
    );
