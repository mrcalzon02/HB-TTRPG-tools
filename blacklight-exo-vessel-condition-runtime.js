(() => {
  ...
  function apply(input,result){
    ...
    result.warnings=[...(result.warnings||[]),`VESSEL-05 applied ${history.activeTemplate} through ${history.events.length} deterministic history events and retained ${history.recalculated.residualMassTonnes.toLocaleString(undefined,{maximumFractionDigits:3})} tonnes across ${history.residualVoxelField.occupiedPlacementCount} occupied reference placements.`,`The intact module graph and voxel layout remain design authority. Condition history records surviving, absent, and wreckage placement state while combat tracks, weapon envelopes, and local hit resolution remain deferred to VESSEL-06 through VESSEL-08.`];
    return result;
  }
