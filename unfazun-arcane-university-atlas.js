(() => {
  ...
  const layerButton=document.getElementById('unfazun-map-layer');
  ...
  function mix(hex,amount){const number=Number.parseInt(hex.replace('#',''),16);const channel=shift=>Math.max(0,Math.min(255,((number>>shift)&255)+amount));return`#${[channel(16),channel(8),channel(0)].map(value=>Math.round(value).toString(16).padStart(2,'0')).join('')}`;}
  ...
  function installMapControls(){
    ...
    if(layerButton)layerButton.addEventListener('click',()=>{const node=nodeById.get(state.selected);if(!node)return;state.district=String(node.district);districtFilter.value=state.district;fitVisible();renderInfo();});
