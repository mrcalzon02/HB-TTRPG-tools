(() => {
  ...
  function biomes(profile) {
    ...
    if (/jungle|forest|maiden|exodite|feral|paradise|swamp/.test(template)) return ['temperate forest','humid lowlands','rocky highlands'];
  ...
  function create(node, records = []) {
    ...
    if (!engine?.createProfile || !engine?.templateForBody || !engine?.templateForMoon) throw new Error('Planetary profile engine is unavailable to the system survey register.');
  ...
  function renderCensus(survey, options = {}) {
    ...
    heading.textContent = 'Generated Local-System Survey Census'; intro.className = entry ? 'wh-entry-copy' : 'wh-small'; intro.textContent = 'The Navis docket and Administratum dossier read from this same deterministic survey register.'; summary.className = entry ? 'wh-entry-ledger' : 'wh-definition';
    addDef(summary,'System survey class',label(survey.template)); addDef(summary,'Planetary bodies',survey.bodies.length); addDef(summary,'Natural satellites',survey.bodies.reduce((total,body)=>total+body.moons.length,0)); addDef(summary,'Asteroid belts',survey.beltCount); addDef(summary,'Orbital stations',survey.stationCount); addDef(summary,'Fleet anchorages',survey.anchorageCount); section.append(heading,intro,summary);
    survey.bodies.forEach((body,index)=>{
      const details=document.createElement('details'), title=document.createElement('summary'), dl=document.createElement('dl'); details.className='wh-system-survey-body'; details.open=body.registered||index===0; title.textContent=`${body.name}${body.registered?' · Registered world':''}`; dl.className=entry?'wh-entry-ledger':'wh-definition';
      addDef(dl,'World classification',label(body.template)); addDef(dl,'Atmosphere',body.survey.atmosphere); addDef(dl,'Dominant biomes',body.survey.biomes); addDef(dl,'Mean surface temperature',`${body.survey.temperatures.meanC} °C · ${body.survey.temperatures.meanK} K`); addDef(dl,'Observed temperature range',`${body.survey.temperatures.lowC} to ${body.survey.temperatures.highC} °C`); addDef(dl,'Equatorial survey temperature',`${body.survey.temperatures.equatorialC} °C`); addDef(dl,'Polar survey temperature',`${body.survey.temperatures.polarC} °C`); addDef(dl,'Biosphere index',`${Math.round(body.survey.biosphereIndex*100)}%`); addDef(dl,'Biosphere status',body.survey.biosphereStatus); addDef(dl,'Recorded wildlife',body.survey.recordedWildlife.length?body.survey.recordedWildlife:'No fauna entered in the current survey'); addDef(dl,'Environmental hazards',body.survey.hazards.length?body.survey.hazards:'No exceptional environmental hazard entered'); addDef(dl,'Habitability',body.survey.habitability); addDef(dl,'Axial period',`${body.rotationPeriodHours.toFixed(1)} hours`); addDef(dl,'Orbital period',`${body.orbitalPeriodHours.toFixed(1)} hours · local survey scale`); addDef(dl,'Ring system',body.rings?'Confirmed':'None recorded'); details.append(title,dl);
      body.moons.forEach(moon=>{const box=document.createElement('section'), h=document.createElement(entry?'h4':'h5'), mdl=document.createElement('dl'); box.className='wh-system-survey-moon'; h.textContent=moon.name; mdl.className=entry?'wh-entry-ledger':'wh-definition'; addDef(mdl,'Satellite classification',label(moon.template)); addDef(mdl,'Atmosphere',moon.survey.atmosphere); addDef(mdl,'Dominant biomes',moon.survey.biomes); addDef(mdl,'Mean temperature',`${moon.survey.temperatures.meanC} °C · ${moon.survey.temperatures.meanK} K`); addDef(mdl,'Observed temperature range',`${moon.survey.temperatures.lowC} to ${moon.survey.temperatures.highC} °C`); addDef(mdl,'Biosphere status',moon.survey.biosphereStatus); addDef(mdl,'Recorded wildlife',moon.survey.recordedWildlife.length?moon.survey.recordedWildlife:'No fauna entered in the current survey'); addDef(mdl,'Environmental hazards',moon.survey.hazards.length?moon.survey.hazards:'No exceptional environmental hazard entered'); addDef(mdl,'Axial period',`${moon.rotationPeriodHours.toFixed(1)} hours`); addDef(mdl,'Orbital period',`${moon.orbitalPeriodHours.toFixed(1)} hours · local survey scale`); box.append(h,mdl); details.append(box);}); section.append(details);
    });
    return section;
  }
