...
/** Deterministic passive progression for schema-029 settlement projects. */
public final class SettlementProjectEngine {
    ...
    private static List<Project> readProjects(Connection connection, String worldId) throws SQLException {
        ...
        try (PreparedStatement statement = connection.prepareStatement(
                "SELECT project_id,status,origin_station_id,target_station_id,required_security,current_security,"
                        + "committed_material_units,committed_supply_units,committed_population,"
                        + "committed_transport_units,progress_units,target_progress_units "
                        + "FROM settlement_project WHERE world_id=? AND status IN ('ACTIVE','BLOCKED') "
                        + "ORDER BY project_id")) {
    ...
    private static Project read(ResultSet result) throws SQLException {
        return new Project(result.getString(1), result.getString(2), result.getString(3), result.getString(4),
                ...
                result.getInt(10), result.getInt(11), result.getInt(12));
    }
    ...
    private record Project(String projectId, String status, String originStationId, String targetStationId,
                           int requiredSecurity, int currentSecurity, int committedMaterialUnits,
                           int committedSupplyUnits, int committedPopulation, int committedTransportUnits,
                           int progressUnits, int targetProgressUnits) { }

    public record EngineResult(int eligibleProjects, int advancedProjects, int blockedProjects,
                               int resumedProjects, int completedProjects) { }
}
